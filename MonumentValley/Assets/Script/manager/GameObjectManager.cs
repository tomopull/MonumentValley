...
//			Vector3 lookPos = Camera.main.ScreenToWorldPoint(mousePos);
//			lookPos = lookPos - _hero.transform.position;
//			float angle = Mathf.Atan2(lookPos.y, lookPos.x) * Mathf.Rad2Deg;
//			_hero.transform.rotation = Quaternion.AngleAxis(angle, Vector3.forward);
//
//
//		}
//	
...
//		//マウスポインターが何らかのEventSystem関連のUI用のGameObject上になかった場合
//		if(!EventSystem.current.IsPointerOverGameObject()){
//			cameraDif = Camera.main.transform.position.y - _hero.transform.transform.position.y;
//			mouseX = Input.mousePosition.x;
//			mouseY = Input.mousePosition.y;
//			worldpos = Camera.main.ScreenToWorldPoint(new Vector3(mouseX, mouseY, cameraDif));
//			Vector3 turretLookDirection = new Vector3 (worldpos.x,_hero.transform.position.y, worldpos.z);
//			_hero.transform.LookAt(turretLookDirection);
//		}
//
//	}
